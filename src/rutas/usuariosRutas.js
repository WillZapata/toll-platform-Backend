...
const {
  usuariosInternosModelos,
} = require("../modelos/usuariosInternosModelos");
...
usuariosRutas.post("/guardar", userAuthguard, function (req, res) {
  ...
  user.save(function (error) {
    if (error) {
      return res
        .status(500)
        .json({ estado: "error", msg: "Usuario no guardado" });
    }
...
usuariosRutas.post("/registro", function (req, res) {
  ...
  ext.save(function (error) {
    if (error) {
      return res
        .status(500)
        .json({ estado: "error", msg: "Error al registrarse ", error });
    }
...
usuariosRutas.patch("/actualizar/:id", async function (req, res) {
  ...
  try {
    ...
    if (body.contrasena) {
      const newPassword = await encryptPassword(body.contrasena);
      newData = { ...newData, contrasena: newPassword };
    }

    await usuariosInternosModelos.findOneAndUpdate(id, newData);

    return res.status(200).send({ estado: "OK", msg: "Usuario actualizado" });
  } catch (error) {
    res.status(500).json({ estado: "error", msg: "Algo salio mal", error });
  }
});
